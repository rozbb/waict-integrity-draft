...
* `inline_js_hashes` (optional) — a sorted list of unique SHA-256 hashes (base64urlnopad) of permitted inline script element content and inline event-handler attribute values. The hash is computed over the same byte sequence that CSP3's [hash-source](https://www.w3.org/TR/CSP3/#grammardef-hash-source) check would hash. See [Inline Scripts, Styles, and Dynamic Code](#inline-scripts-styles-and-dynamic-code).
...
The user-agent parses this string with [parse a serialized CSP](https://w3c.github.io/webappsec-csp/#parse-serialized-policy), using disposition `"enforce"` when `mode-inline-js` is `enforce` and disposition `"report"` when it is `warn`, and appends the resulting policy to the document's CSP list. Violations of this synthesized policy are surfaced as a WAICT `inline_violation` (see [Failure Handling](#failure-handling)) rather than as an ordinary `csp-violation`. Enforcing the policy drives the standard CSP3 algorithms (extended by script-src-v2 for the eval-hash source):
...
Navigation to a `javascript:` URI is also gated on `mode-inline-js`, since CSP3 treats it as inline-script execution and the synthesized `script-src` therefore blocks it. WAICT applies one carve-out the serialized policy cannot express: when `mode-inline-js` is `enforce` or `warn`, the user-agent MUST NOT apply the synthesized policy's block to a `javascript:` navigation whose [`userInvolvement`](https://html.spec.whatwg.org/multipage/browsing-the-web.html#beginning-navigation) is `"Browser UI"`. The manifest provides no allowlist for `javascript:` URIs; CSP3 has no hash mechanism for them. The Browser-UI carve-out preserves JavaScript bookmarklets while blocking page-triggered navigation.
...
The user-agent parses this string with [parse a serialized CSP](https://w3c.github.io/webappsec-csp/#parse-serialized-policy), using disposition `"enforce"` when `mode-inline-css` is `enforce` and disposition `"report"` when it is `warn`, and appends the resulting policy to the document's CSP list. Violations are surfaced as a WAICT `inline_violation` (see [Failure Handling](#failure-handling)). Enforcing the policy drives the standard CSP3 ["Should element's inline type behavior be blocked?"](https://www.w3.org/TR/CSP3/#should-block-inline) algorithm for types `"style"` and `"style attribute"`: inline `<style>` elements and inline `style` attributes are blocked, except when the SHA-256 hash of the element's content (or attribute value) appears in `manifest["inline_css_hashes"]`. A `nonce` attribute or a CSP-supplied hash-source never permits execution under WAICT; only manifest-listed hashes do.
