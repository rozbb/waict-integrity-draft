...
* `inline_js_hashes` (optional) — a sorted list of unique SHA-256 hashes (base64urlnopad) of permitted inline script element content, inline event-handler attribute values, and `javascript:` URI script sources. The hash is computed over the same byte sequence that CSP3's [hash-source](https://www.w3.org/TR/CSP3/#grammardef-hash-source) check would hash. See [Inline Scripts, Styles, and Dynamic Code](#inline-scripts-styles-and-dynamic-code).
...
* `inline_css_hashes` (optional) — a sorted list of unique SHA-256 hashes (base64urlnopad) of permitted inline `<style>` element content and inline `style` attribute values, computed identically to `inline_js_hashes` but for styles. See [Inline Scripts, Styles, and Dynamic Code](#inline-scripts-styles-and-dynamic-code).
...
Each hash list above (`wasm_hashes`, `fallback_hashes`, `wildcard_hashes`, `inline_js_hashes`, `eval_hashes`, `inline_css_hashes`, and `inline_url_hashes`) MUST be sorted in ascending lexicographic order. The sorted order enables efficient membership testing by user-agents.
...
WAICT addresses these paths through additional restrictions whose strictness is controlled by the mode keys `mode-inline-js` and `mode-inline-css` defined in [Response Header](#response-header). Rather than editing the source expressions of whatever CSP the origin happens to deliver, the user-agent **synthesizes its own Content Security Policy**: it constructs a serialized policy string from the manifest and the mode keys, parses that string with CSP3's [parse a serialized CSP](https://w3c.github.io/webappsec-csp/#parse-serialized-policy) algorithm, and appends the resulting policy to the CSP list used to enforce the document. This is the same machinery the HTML Standard uses to install a policy delivered through a [`<meta http-equiv="content-security-policy">`](https://html.spec.whatwg.org/#attr-meta-http-equiv-content-security-policy) element, which parses its `content` attribute with the same algorithm and appends the policy to the document's policy container. Synthesizing a policy lets user-agents reuse the existing CSP enforcement code paths without mutating the parsed representation of the origin's CSP.
...
When `mode-inline-js` is set to `enforce` or `warn`, the user-agent MUST synthesize a CSP carrying a single `script-src` directive and enforce it as described above. The serialized policy string is constructed as:

```
script-src * data: blob: filesystem: 'wasm-unsafe-eval' 'unsafe-hashes' <H>… <E>…
```

where:

* the host-source `*` together with the scheme-sources `data:`, `blob:`, and `filesystem:` permit every external and scheme-based script source, so that the synthesized policy governs only inline execution and string compilation. It never blocks a network fetch (whose integrity is covered by the fetch-based check) nor a `data:`/`blob:` active-content load (covered by [`data:` and `blob:` URLs as Active Content](#data-and-blob-urls-as-active-content)); those loads remain subject to the origin's own CSP and to WAICT's other checks, which are enforced as separate conjuncts.
* `'wasm-unsafe-eval'` is included so the synthesized policy does not block WebAssembly compilation, which is governed independently by `mode-wasm` (see [Changes to WebAssembly Processing](#changes-to-webassembly-processing)).
* `<H>…` is a SHA-256 hash-source (`'sha256-…'`) for every entry in `manifest["inline_js_hashes"]`, if that field is present.
* `<E>…` is a SHA-256 eval-hash source (`'eval-sha256-…'`; see [script-src-v2](https://github.com/explainers-by-googlers/script-src-v2)) for every entry in `manifest["eval_hashes"]`, if that field is present.
* `'unsafe-hashes'` is included so the hash-sources match inline event-handler attribute values as well as inline `<script>` element content (see below).
* the policy carries no `'unsafe-inline'`, no `'unsafe-eval'`, and no nonce-source, so inline `<script>` elements, inline event-handler attributes, and string compilation are permitted only when their content matches a manifest-supplied hash.

The user-agent parses this string with [parse a serialized CSP](https://w3c.github.io/webappsec-csp/#parse-serialized-policy), using disposition `"enforce"` when `mode-inline-js` is `enforce` and disposition `"report"` when it is `warn`, and appends the resulting policy to the document's CSP list. From there the synthesized policy is an ordinary CSP, enforced by standard CSP3 processing unchanged; its violations are surfaced as a WAICT `inline_violation` (see [Failure Handling](#failure-handling)) rather than as an ordinary `csp-violation`.
...
Navigation to a `javascript:` URI is also gated on `mode-inline-js`, since CSP3 treats it as inline-script execution. CSP3 matches a `javascript:` navigation target against `script-src` hash-sources when `'unsafe-hashes'` is present (the same keyword that lets the synthesized policy cover event-handler attributes), hashing the script source the URI carries. A `javascript:` navigation is therefore permitted exactly when the SHA-256 of that script source appears in `manifest["inline_js_hashes"]`, and blocked otherwise — the same allowlisting rule WAICT applies to inline `<script>` elements and event-handler attributes, and it falls out of the synthesized policy with no extra machinery.

WAICT adds one carve-out the serialized policy cannot express: when `mode-inline-js` is `enforce` or `warn`, the user-agent MUST permit a `javascript:` navigation whose [`userInvolvement`](https://html.spec.whatwg.org/multipage/browsing-the-web.html#beginning-navigation) is `"Browser UI"` even when its hash is absent from the manifest. This preserves JavaScript bookmarklets while still blocking page-triggered `javascript:` navigations that the manifest does not list.

Because the synthesized policy sets only `script-src`, CSP3 derives `script-src-elem` and `script-src-attr` from it, and the `"Should element's inline type behavior be blocked?"` algorithm consults the appropriate derived directive for each check (`script-src-elem` for element content, `script-src-attr` for attribute values). The same rules apply uniformly to classic scripts, module scripts, and importmaps.

CSP3's hash-source matching applies to inline `<script>` element content by default, but applies to inline event-handler attribute values only when `'unsafe-hashes'` is present — which is why the synthesized policy includes it. A single entry in `inline_js_hashes` therefore permits both an inline `<script>` element and an inline event-handler attribute with the same content bytes. Because the synthesized policy is the only policy carrying these hash-sources, this treatment never widens what the origin's own CSP allows.
...
When `mode-inline-css` is set to `enforce` or `warn`, the user-agent MUST synthesize a CSP carrying a single `style-src` directive and enforce it as described above. The serialized policy string is constructed as:

```
style-src * data: blob: filesystem: 'unsafe-hashes' <H>…
```

where the host-source `*` and the scheme-sources `data:`, `blob:`, and `filesystem:` permit every external and scheme-based style source so that the synthesized policy governs only inline styles; `<H>…` is a SHA-256 hash-source (`'sha256-…'`) for every entry in `manifest["inline_css_hashes"]`, if that field is present; `'unsafe-hashes'` makes those hash-sources match inline `style` attribute values as well as `<style>` element content (see below); and the policy carries no `'unsafe-inline'` and no nonce-source.

The user-agent parses this string with [parse a serialized CSP](https://w3c.github.io/webappsec-csp/#parse-serialized-policy), using disposition `"enforce"` when `mode-inline-css` is `enforce` and disposition `"report"` when it is `warn`, and appends the resulting policy to the document's CSP list. From there the synthesized policy is an ordinary CSP, enforced by standard CSP3 processing unchanged; its violations are surfaced as a WAICT `inline_violation` (see [Failure Handling](#failure-handling)).

Because the synthesized policy sets only `style-src`, CSP3 derives `style-src-elem` and `style-src-attr` from it, and the algorithm consults the appropriate derived directive for each check (`style-src-elem` for `<style>` element content, `style-src-attr` for `style` attribute values).

CSP3's hash-source matching applies to inline `<style>` element content but does not natively apply to inline `style` attribute values (CSP3 requires `'unsafe-hashes'` for the latter) — which is why the synthesized policy includes `'unsafe-hashes'`. A single entry in `inline_css_hashes` therefore permits both an inline `<style>` element and an inline `style` attribute with the same content bytes. The hash is computed over the same byte sequence that CSP3's hash-source check would hash for an inline `<style>` element; for a `style` attribute, the hash is computed over the attribute value's bytes (UTF-8 encoded). Because the synthesized policy is the only policy carrying these hash-sources, this treatment never widens what the origin's own CSP allows.
